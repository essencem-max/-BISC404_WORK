...
```{butterfly_df <- read_excel("ecog12731-sup-0001-appendixs1.xlsx",}
                           sheet = "butterfly_abundance_data")
...
```{=plaintext}
The dataset contains butterfly abundance data collected from islands in the Boundary Waters region. Each row represents an island, while columns include island characteristics such as area and isolation, as well as presence/absence or abundance data for multiple butterfly species and their associated host plants. The dataset is in a wide format with many columns representing different species.
The dataset needs cleaning because it contains many columns that are difficult to work with in wide format, possible missing values (NA), and inconsistent column naming between butterfly species and host plant data. Additionally, species richness is not directly provided and must be calculated. The dataset also includes non-resident species, which may inflate richness values and should be adjusted using host plant availability.
```
#### Clean the dataset:

```{r}
# Clean column names
names(butterfly_df) <- make.names(names(butterfly_df))

# Remove missing values
butterfly_df_clean <- butterfly_df %>%
  drop_na()
...
```{=plaintext}
According to Island Biogeography Theory, species richness is expected to increase with island area and decrease with island isolation. Larger islands support more species due to greater habitat availability, while more isolated islands have fewer species because of reduced colonization rates. Area and isolation are generally independent of one another.
```
...
```{r}
ggplot(butterfly_df_clean, aes(x = area, y = isolation_1)) +
  geom_point() +
  geom_smooth(method = "lm", se = FALSE) +
  labs(title = "Area vs Isolation")
