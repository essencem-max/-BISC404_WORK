...
# Define parameters
p <- 100      # total species pool
c <- 0.1      # colonization rate
h <- 0.05     # extinction rate
...
ggplot(data = df, ase(x=s))+
  geom_line(aes(y = C, color = "Colonization Rate")) +
  geom_line(aes(y = E, color = "Extinction Rate")+
              geom_vline(xintercept = s_star)
...
names(lakshadweep_AGATHI_clean) <- 
lakshadweep_AGATHI_clean %>% 
  filter(row_number () == 1)
...
names(lakshadweep_KADMATH_clean) <- 
lakshadweep_KADMATH_clean %>% 
  filter(row_number () == 1)
...
names(lakshadweep_KAVRATTI_clean) <- 
lakshadweep_KAVRATTI_clean %>% 
  filter(row_number() == 1)
...
ggplot(df, aes(x = S)) +
  ...
  labs(title = "Colonization and Extinction Rates",
...
D_AGATHI <- 459 # distance of Agathi island
A_AGATHI <- 3.84     # area of Agathi island
...
df_AGATHI_extended <- data.frame(S = S_values, C = C_values, E = E_values)
...
parms <- c(c = 0.15, m = 0.06)
P0 <- 0.01
times <- 1:100

out <- ode(y = P0, times = times, func = levins, parms = parms)
...
ggplot(out_df, aes(x = time, y = P1)) +
  geom_line() +
  labs(title = "Levin's Metapopulation Model",
       x = "Time",
       y = "Proportion of Occupied Patches") +
  theme_minimal()
